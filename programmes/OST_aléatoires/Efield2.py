...
def Efield(x,y,z,I,f):
    ...
    for i in range(0,N): 
        ...
        cosphi   = distx/distxy
        sinphi   = disty/distxy
        L =tile(I[:,5],(1,1))*1/dp*(fp.T/c)**2*377
...
def Efarfield(R,theta,phi,I,f):
    ...
    for i in range(0,N): 
        ...
        cosphi   = distx/distxy
        sinphi   = disty/distxy
        L =I[:,5]*1/dp*(fp/c)**2*377
        Exx = sum(exp(1j*phase)*L*((((-sb)**2+(1-(-sb)**2)*ca)\
            *(-sintheta*costheta*cosphi)+(-sb*cb*(1-ca))\
            *(-sintheta*costheta*sinphi)+(-cb*sa)\
            *(-sintheta*(-sintheta)))))
        Eyy = sum(exp(1j*phase)*L*((((-sb*cb*(1-ca))\
            *(-sintheta*costheta*cosphi)+((cb)**2+(1-(cb)**2)*ca)\
            *(-sintheta*costheta*sinphi)+(-sb*sa)\
            *(-sintheta*(-sintheta))))))
        Ezz = sum(exp(1j*phase)*L*(((cb*sa)\
            *(-sintheta*costheta*cosphi)+(sb*sa)\
            *(-sintheta*costheta*sinphi)\
            +ca*(-sintheta*(-sintheta)))))           
        Eth[i]= Exx*cos(theta[i])*cos(phi[i])\
            +Eyy*cos(theta[i])*sin(phi[i])-Ezz*sin(theta[i])
        Eph[i]= -Exx*sin(phi[i])+Eyy*cos(phi[i])
        #Er[i,:] = Exx*sin(theta[i])*cos(phi[i])\
